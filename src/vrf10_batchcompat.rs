...
mod test {
    ...
    #[bench]
    fn bench_hash_to_curve(b: &mut Bencher) {
        let mut alpha = vec![0u8; 32];
        let mut rng = ChaCha20Rng::from_seed([0u8; 32]);
        rng.fill_bytes(&mut alpha);
        let secret_key = SecretKey10::generate(&mut rng);
        ...
        b.iter( || {
                VrfProof10BatchCompat::hash_to_curve(&public_key, &alpha)
            });
    }

    #[bench]
    fn bench_nonce_generations(b: &mut Bencher) {
        ...
        b.iter( || {
            VrfProof10BatchCompat::nonce_generation10(alpha, public_key.0);
        });
    }

    #[bench]
    fn bench_compute_challenge(b: &mut Bencher) {
        let mut alpha = [0u8; 32];
        let mut rng = ChaCha20Rng::from_seed([0u8; 32]);
        rng.fill_bytes(&mut alpha);
        let compressed_h = EdwardsPoint::hash_from_bytes::<Sha512>(&alpha).compress();
        rng.fill_bytes(&mut alpha);
        let gamma = EdwardsPoint::hash_from_bytes::<Sha512>(&alpha);
        rng.fill_bytes(&mut alpha);
        let announcement_1 = EdwardsPoint::hash_from_bytes::<Sha512>(&alpha);
        rng.fill_bytes(&mut alpha);
        let announcement_2 = EdwardsPoint::hash_from_bytes::<Sha512>(&alpha);
